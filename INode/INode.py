...
class CommandCall:
    def __init__(self, timeout, command, agent_config, checks_logger,
                 raw_config):
        self.agentConfig = agent_config
        self.checksLogger = checks_logger
    ...
    def parse_response(self, data):
        ...
        raise NotImplementedError(
            "Implement this method to parse the output of the command")

    def handler(self, signal_number, interrupted_stack_frame):
        # this handler will be called if the command could not return results
        # in self.timeout seconds
        raise IOError("Could not execute command {0} in {1} seconds".format(
            str(self.command), str(self.timeout)))
    ...
    def run(self):
        ...
        try:
            # self.command is the command that will be executed at the forked
            # child process preexec_fn=os.setsid will add a session id to
            # forked process tree's root process this id will be
            # used to kill all the processes in the tree if the root process
            # of tree received a kill call/signal
            ...
            # this will arrange an alarm signal to arrive after the given
            # number of seconds
            signal.alarm(self.timeout)

            # communicate() will read data from stdout and stderr, until
            # end-of-file is reached and it will wait for process to terminate,
            # this may be a problem for some cases like OS level or
            # driver errors, ex. corrupt or not responding remote mounted
            # filesystems may took very long time for commands like df, ls or
            # they may block altogether, so the signal.alarm()
            # is used for a timeout.
            (result, error) = command_process.communicate()
            ...
            try:
                ...
                # uncomment below line if you would like to see this at
                # dashboard data['exception'] = 'OSError, process already
                #                       terminated, probably communicate()
                #                       method ended the process, this is OK'
                pass
            except Exception, e:
                # this exception is not expected, it might be important
                # if the forked child processes do not terminate
                # as expected and accumulate in time,
                # they might consume resources
                data['exception'] = \
                    'Exception at process kill() call: {0} {1}'.format(
                        str(e), str(type(e)))

        except IOError, e:
            # this exception raised from handler method which indicates the
            # command could not finish its job
            # within the self.timeout seconds
            data['exception'] = 'Exception, Alarm raised IOError: {0}'.format(
                str(e))

        except Exception, e:
            # this exception could be anything along the execution stack
            data['exception'] = 'Exception: {0} {1}'.format(
                str(e), str(type(e)))
...
class INode(CommandCall):
    def __init__(self, agent_config, checks_logger, raw_config):
        # sd-agent will call this constructor for this plugins creation,
        ...
        CommandCall.__init__(self, self.timeout, self.command, agent_config,
                             checks_logger, raw_config)
    ...
    def parse_response(self, data):
        ...
        try:
            if data['result'] is not None:
                ...
                # df has first row as the headers like
                # Filesystem, Inodes, IUsed etc.
                headers = None
                for index in range(len(lines)):
                    ...
                    # there will be only one header row,
                    ...
                    values = line.split()
                    # this row(dictionary) will hold the
                    # key values for each line
                    row = {}
                    for headerIndex in range(len(headers)):
                        ...
                        if self.allOrPercentage and "%" in header:
                            ...
                            # return the last value as key which is
                            # something like /, /dev, /home etc.
                            # remove all non numeric values from value,
                            # for some rows the percentage value is not
                            # available, use 0 for them
                            remove_non_numeric = re.compile(r'[^\d.]+')
                            value = remove_non_numeric.sub('', value)
                            if value == "":
                                value = "0"
                            data[values[-1]] = value
                            # break the loop since no need to iterate
                            ...
                            try:
                                ...
                                if headerIndex == len(headers) - 1:
                                    # add this row to data dictionary for
                                    # the latest value which is unique,
                                    # like /, /dev, /home etc.
                                    data[value] = row
                            except IndexError, e:
                                pass
            ...
            data['exception'] = "%s, Exception in INode: %s %s" % (
                data['exception'], str(e), str(type(e)))
...
def main():
    agent_config, checks_logger, raw_config = None, None, None
    inode = INode(agent_config, checks_logger, raw_config)
    inode.run()
